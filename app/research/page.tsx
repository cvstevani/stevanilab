...
const topics: Record<string, Topic> = {
  ...
  "bioanalytical-tools": { title:"Bioanalytical tools", intro:"Bioluminescent organisms and pathways become sensitive tools for quantification, toxicity assessment and environmental monitoring.", articles:[
    ["2026","A Fungal Bioluminescent Pathway-based yeast biosensor for caffeic acid quantification","ACS Sensors","/publications"],
  ...
  "others": { title:"Others", intro:"Research beyond the principal themes, spanning environmental chemistry, oxidative processes, biotechnology and organismal bioluminescence.", articles:[
    ["2022","Patent landscape of polyhydroxyalkanoates production by algae and cyanobacteria","Recent Patents on Biotechnology","/publications"],
    ["2021","Oxidative modification of proteins: from damage to catalysis, signaling and beyond","Antioxidants & Redox Signaling","/publications"],
  ]},
};
...
export default function ResearchTopics() {
  ...
  return <main className="topics-page">
    ...
    <section className="topic-content"><div className="topic-title"><h2>{topic.title}</h2><p>{topic.intro}</p></div><div className="topic-columns">
      <div className="topic-column"><h3>Scientific articles</h3>{topic.articles.map(([year,title,journal,url])=><a className="topic-item" href={url} key={title}><time>{year}</time><h4>{title} ↗</h4><p>{journal}</p></a>)}<a className="topic-all-papers" href="/publications">Browse complete publication list ↗</a></div>
